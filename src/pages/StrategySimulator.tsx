...
import {
    ...
    LayoutDashboard, Search, RotateCcw, EyeOff, Eye,
    PanelLeftClose,
    PieChart
} from "lucide-react";
...
type ColumnId =
    ...
    | "ecoColRatio" | "navRatio" | "ecoColHolderRatio" | "ecoStratHolderRatio" | "stratColHolderRatio";
...
const DEFAULT_HIDDEN_GROUPS = new Set(["Fees", "NFT Collection", "Ecosystem Token", "KPI / Ratio"]);
...
type MergedData = StrategyData & {
    ...
    feesStrat?: number; feesPnkstr?: number; feesRoyalties?: number; ecoColRatio?: number; navRatio?: number;
...
function Sidebar({
    isOpen, onClose, strategies, visibleStrategyIds, toggleStrategy,
    ...
    return (
        <aside
            ref={sidebarRef}
            className={`absolute top-0 left-0 h-full bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col shadow-2xl z-50 transition-transform duration-300 ease-in-out w-72 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
        >
            <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between bg-gray-50 dark:bg-gray-800/50">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-bold text-gray-900 dark:text-white text-sm">Dashboard Controls</h2>
                </div>
                <button onClick={onClose} className="p-1 text-gray-400 hover:text-gray-600 hover:bg-gray-200 rounded-md"><PanelLeftClose size={18} /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
                ...
                <div>
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="text-xs font-bold uppercase text-gray-500 tracking-wider flex items-center gap-2"><Filter size={12} /> Strategies</h3>
                ...
                <div>
                    <div className="flex items-center justify-between mb-3">
                        <h3 className="text-xs font-bold uppercase text-gray-500 tracking-wider flex items-center gap-2"><Columns size={12} /> Data Columns</h3>
                    </div>
...
export default function StrategyDashboard(): JSX.Element {
    ...
    const [isSidebarOpen, setIsSidebarOpen] = useState(false); // Default closed now

    const [visibleStrategyIds, setVisibleStrategyIds] = useState<Set<string>>(new Set());
    const [visibleColumns, setVisibleColumns] = useState<Set<ColumnId>>(() => new Set(COLUMN_DEFS.filter(c => !c.headerGroup || !DEFAULT_HIDDEN_GROUPS.has(c.headerGroup)).map(c => c.id)));
        ...
        const loadMissingData = async () => {
            ...
            for (const strat of toLoad) {
                ...
                try {
                    ...
                    const strategyAddressPadded = padAddress(strat.tokenAddress);
                    let buyUrl = "", sellUrl = "";
                    if (isPunks) {
                    ...
                    setStrategies(prev => prev.map(s => s.id === strat.id ? {
                        ...
                        feesStrat: totalBuyVolume * 0.08, feesPnkstr: totalBuyVolume * 0.01, feesRoyalties: totalBuyVolume * 0.01,
                        realizedPnLEth: realizedPnL, tradesCount: trades.length, tradeHistory: trades, inventoryCount, treasuryValueUsd: treasuryVal,
                        ecoColRatio, navRatio, ecoColHolderRatio, ecoStratHolderRatio, stratColHolderRatio, dataLoaded: true, isLoading: false
                    } : s));
    ...
    const processedData = useMemo(() => {
        ...
        if (sortConfig) {
            data.sort((a, b) => {
                ...
                // Simplistic mapping for brevity, real app should map all fields
        ...
        <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-950 font-sans overflow-hidden">
            ...
            <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4 flex justify-between items-center shrink-0 z-50 relative">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={() => { setVisibleStrategyIds(new Set(strategies.map(s => s.id))); setVisibleColumns(new Set(COLUMN_DEFS.map(c => c.id))); }} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors">
                        ...
                        <thead className="bg-gray-50 dark:bg-gray-800/90 text-xs uppercase text-gray-500 tracking-wider z-30 sticky top-0 shadow-sm">
                            ...
                            <tr>
                                {visibleDefs.map((col) => {
                                    ...
                                    return (
                                        <th key={col.id} onClick={() => handleSort(col.id)} className={`px-4 py-2 cursor-pointer hover:brightness-95 transition border-b border-gray-200 dark:border-gray-700/50 ${bgClass}`}>
                                                    ...
                                                    <>
                                                        ...
                                                        {col.id === "treasury" && (
                                                            <div className="flex flex-col items-end">
                                                                ...
                                                                <div className="text-[10px] text-gray-400">{fmtUSD(s.treasuryValueUsd)}</div>
                                                            </div>
                                                        )}
